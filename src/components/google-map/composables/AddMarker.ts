...
export default function addMarkers(map: google.maps.Map, markerData: brxmFeature) {
    if (markerData.geometry.type === 'Point') {
        ...
        if (markerData.properties.category.id) {
            mapPin.classList.add(`vs-google-map__map-pin--${markerData.properties.category.id}`);
            const mapPinIcon = document.createElement('i');
            let iconClass = (mapIconMapping.methods.getIconDetails(markerData.properties.category.id).name).split(' ');
